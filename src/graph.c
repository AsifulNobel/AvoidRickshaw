...
void cairo_drawing(void *data)
{
	...
	for(int i = 0; i < 7; i++)
	{
		if(fare[i] > maxFare)
			maxFare = fare[i];
		//netFare = netFare + fare[i];
	}
	//double avg = netFare / 7;
	//avg = 0.6 - (avg/maxFare) * 0.5;
	...
	//double avg = (fractionFare[0] + fractionFare[1] + fractionFare[2] + fractionFare[3] + fractionFare[4] + fractionFare[5] + fractionFare[6])/7;
